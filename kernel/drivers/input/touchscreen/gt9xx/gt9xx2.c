...
static struct gtp_config_custom *gtp_config;

static const char *goodix_ts_name = "goodix-ts2";
static const char *goodix_input_phys = "input/ts2";
...
void gtp2_reset_guitar(struct i2c_client *client, s32 ms);
s32 gtp2_send_cfg(struct i2c_client *client);
void gtp2_int_sync(s32 ms);
...
#ifdef GTP_CONFIG_OF
int gtp2_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid);
#endif
...
static long int touch_wakeup_time_gt9xx(void)
{
...
*********************************************************/
static long int time_difference_absolute_gt9xx(long int time_val)
{
	long time_temp = 0;
...
{
     const int min_touch_back = (ts->abs_x_max * 2 )/ 5;
     const int max_touch_back = (ts->abs_x_max * 3 )/ 5;
	...
	if(gtp_config->gtp_touch_back == 1)
		{
            ...
            if ( click_index == 1 && 
                (click_array[1] - click_array[0]) > min_touch_back && 
                (click_array[1] - click_array[0]) < max_touch_back )
            {
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    while(retry++ < 10)
    {
        ...
        gtp2_reset_guitar(ts->client, 20);
    }
#endif
...
//#if GTP_CONFIG_OF	
	...
	printk("sensor_id = %d\n",sensor_id);
	ret = gtp2_parse_dt_cfg(&ts->client->dev, &config[GTP_ADDR_LENGTH], &ts->gtp_cfg_len, sensor_id);
	if (ret < 0) {
...
static ssize_t gt91xx_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    ...
    ret = gtp2_send_cfg(i2c_connect_client);
...
static void get_ICfrimware(void)
{
...
Output:
    read operation return.
        2: succeed, otherwise: failed
*******************************************************/
s32 gtp2_read_version(struct i2c_client *client, u16* version)
{
    s32 ret = -1;
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    GTP_GPIO_REQUEST(gtp_rst_gpio, "GTP RST PORT");
    if (ret < 0) 
    {
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    ts->input_dev->id.bustype = BUS_USB;	//BUS_USB
    ts->input_dev->id.vendor = 0xDEAD;
    ts->input_dev->id.product = 0xBEEF;
    ts->input_dev->id.version = 10428;		// 10428
...
static void gtp_parse_dt(struct device *dev)
{
	...
	of_property_read_u32(np, "gtp_touch_back", &(gtp_config->gtp_touch_back));
...
//    GTP_INFO("GTP Driver Built@%s, %s", __TIME__, __DATE__);
    ...
    ret = gtp2_read_version(client, &version_info);
    if (ret < 0)
    {
