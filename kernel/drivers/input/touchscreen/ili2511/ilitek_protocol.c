...
int grid_alloc(struct ilitek_ts_device *dev, struct grids *grid)
{
	int X, Y, key;
...
static int api_protocol_get_sensor_id(struct ilitek_ts_device *dev, void *data)
{
	...
	TP_MSG(dev->id, "[Sensor ID] header: 0x%hx, id: 0x%hhx\n",
		dev->sensor.header, dev->sensor.id);
...
static int api_protocol_get_ap_crc_v6(struct ilitek_ts_device *dev, void *data)
{
	...
	//TP_MSG(dev->id, "[FW CRC] Master: 0x%hx\n", dev->ic[0].crc[0]);

	for (i = 1; i < ic_num; i++) {
		dev->ic[i].crc[0] = le16(dev->rbuf + 2 * i);
		//TP_MSG(dev->id, "[FW CRC] Slave[%hhu]: 0x%hx\n", i, dev->ic[i].crc[0]);
	}
...
uint16_t api_get_block_crc_by_num(struct ilitek_ts_device *dev, uint8_t type,
				  uint8_t block_num)
{
	memset(dev->wbuf, 0, 64);
...
int api_read_flash(struct ilitek_ts_device *dev, uint8_t *buf,
		   uint32_t start, uint32_t len)
{
	if (dev->protocol.flag == PTL_V3)
...
int api_read_mp_result(struct ilitek_ts_device *dev, uint8_t *buf, int buf_size)
{
	int error;
	struct tuning_para_settings tuning;
