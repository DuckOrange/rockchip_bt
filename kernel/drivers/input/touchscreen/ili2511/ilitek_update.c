...
static void decode_mm(struct ilitek_fw_handle *fw, uint32_t addr,
		      uint8_t *buf, uint32_t buf_size)
{
	...
	if (le32(mapping->mapping_ver, 3) < 0x010002) {
		_sprintf(fw->file.ic_name, 0, "%02x%02x",
			mapping->ic_name[1], mapping->ic_name[0]);
	} else {
		memcpy(fw->file.ic_name, mapping->ic_name,
...
int ilitek_update_load_fw(void *handle, char *fw_name)
{
	...
	if (strcmp(fw->dev->mcu_info.ic_name, fw->file.ic_name)) {
