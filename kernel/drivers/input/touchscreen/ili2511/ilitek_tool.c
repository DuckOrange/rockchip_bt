...
static ssize_t ilitek_file_write(struct file *filp, const char *buf, size_t size, loff_t *f_pos)
{
	...
	} else if (strcmp(buffer, "dbg_pkt") == 0) {
	...
	TP_DBG(NULL, "ilitek return count = %d\n", (int32_t)size);
	kfree(data);
	return size;
}
				    ...
				    size_t size, loff_t *f_pos)
{
	...
	TP_MSG(NULL, "wlen: %d, rlen: %d, command: %s\n", wlen, rlen, str);

	while ((ptr = strsep(&cur, "-")))
		sscanf(ptr, "%hhx", &cmd[_wlen++]);

	if (wlen > 0 && wlen != _wlen) {
		TP_ERR(NULL, "write cmd length: %d not match with %s\n", wlen, str);
		return -EINVAL;
	}

	if (wlen > 0)
		TP_MSG(NULL, "[write]: %*phD, len: %d\n", wlen, cmd, wlen);

	if ((error = ilitek_write_and_read(cmd, wlen, 1, data, rlen)) < 0)
		return error;

	if (rlen > 0)
		TP_MSG(NULL, "[read]: %*phD, len: %d\n", rlen, data, rlen);
