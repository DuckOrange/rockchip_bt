...
static int pwm_backlight_probe(struct platform_device *pdev)
{
	...
	// kickpi multi lcd backlight 
	dev_set_name(&pdev->dev, "backlight");
	while (max_panel) {
		...
		if (IS_ERR(bl)) {
			max_panel--;			
			if (max_panel == 3) {
				dev_set_name(&pdev->dev, "backlight1");
			} else if (max_panel == 2) {
				dev_set_name(&pdev->dev, "backlight2");
			} else if (max_panel == 1) {
				dev_set_name(&pdev->dev, "backlight3");
			} else if (max_panel == 0) {
